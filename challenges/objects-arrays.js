...
const universities = [];
graduates.forEach((id) => {
	return universities.push(id.university);
});
...
const contactInfo = [];
graduates.forEach((id) => {
	return contactInfo.push(id.first_name + ' ' + id.email);
});
...
zooAnimals.forEach((id) => {
	return displayNames.push(
		`Name: ${id.animal_name}, Scientific: ${id.scientific_name}`
	);
});
